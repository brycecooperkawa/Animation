...
{
    Actor actor(L"Quincy");

    wxPoint point=wxPoint(0,0);

    //test default point
    ASSERT_EQ(point,actor.GetPosition());

    //set new point
    wxPoint point2=wxPoint(999,32);
    actor.SetPosition(point2);

    //test new point
    ASSERT_EQ(point2,actor.GetPosition());
}
