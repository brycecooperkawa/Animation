...
{
    PolyDrawable drawable(L"Hank");
    ASSERT_EQ(std::wstring(L"Hank"), drawable.GetName());
}

TEST(PolyDrawableTest, Color)
{
    PolyDrawable drawable(L"Fizz");

    //test default value
    ASSERT_EQ(*wxBLACK,drawable.GetColor());

    //set new color, test setter and getter
    wxColor color = *wxRED;
    drawable.SetColor(color);
    ASSERT_EQ(color,drawable.GetColor());
}
